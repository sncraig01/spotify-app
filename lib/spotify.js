...
let CHECK_TOKEN_ENDPOINT = `https://api.spotify.com/v1/me`;
export const accessTokenIsValid = async (access_token) => {
  console.log( access_token )
  let response = await callAPI(CHECK_TOKEN_ENDPOINT, access_token)

  if( response.status == 200 ) {
    return true;
  }
  // 401 id unauth, but sould probably return false for other errors as well
  return false
